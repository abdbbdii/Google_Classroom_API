...
async def send_request(session, item_data):
    print("Item Data:", item_data)  # Print item data directly
    async with session.post(
        # appSettings.webhook_url,
        "https://abd-utils-server.vercel.app/this",
        headers={"Content-Type": "application/json"},
        json=item_data
    ) as response:
...
async def get_new_item(session, service, course, item_type, last_check):
    ...
    for item in items.get(item_type, []):
        ...
        if update_time > last_check:
            ...
            is_new = (parse_datetime(item["creationTime"]) - parse_datetime(item["updateTime"])).total_seconds() < 300
            item_data = {"course": course, "activity": item, "type": item_type, "is_new": is_new}
